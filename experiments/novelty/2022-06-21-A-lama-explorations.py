...
CONFIGS = [
    (f"lama-novelty-{novelty}{'-random' if random else ''}{'-reset' if reset else ''}-{handle_progress}", [
        "--evaluator",
        "hlm=lmcount(lm_factory=lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(one),pref=false)",
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--search", f"""lazy(alt([single(hff), single(hlm), novelty_open_list(novelty(width={novelty}, consider_only_novel_states=true, reset_after_progress={reset}), break_ties_randomly={random}, handle_progress={handle_progress})]),preferred=[hff,hlm], cost_type=one,reopen_closed=false)"""])
    for novelty in [2]
    for reset in [True]
    ...
    (f"lama-type-based", [
        ...
        "hlm=lmcount(lm_factory=lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(one),pref=false)",
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--search", """lazy(alt([single(hff), single(hlm), type_based([hff, g()])]),preferred=[hff,hlm], cost_type=one,reopen_closed=false)"""])
] + [
    (f"lama-type-based-novelty-{novelty}{'-random' if random else ''}{'-reset' if reset else ''}-{handle_progress}", [
        "--evaluator",
        "hlm=lmcount(lm_factory=lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(one),pref=false)",
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--search", f"""lazy(alt([single(hff), single(hlm), type_based([hff, g()]), novelty_open_list(novelty(width={novelty}, consider_only_novel_states=true, reset_after_progress={reset}), break_ties_randomly={random}, handle_progress={handle_progress})]),preferred=[hff,hlm], cost_type=one,reopen_closed=false)"""])
    for novelty in [2]
    for reset in [True]
    for handle_progress in ["clear", "move"]
    for random in [False]
