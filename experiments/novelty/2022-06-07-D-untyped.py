...
CONFIGS = [
    ...
    (f"ff-novelty-{novelty}{'-random' if random else ''}{'-reset' if reset else ''}{'-clear' if clear else ''}", [
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--search", f"""lazy(alt([single(hff), novelty_open_list(novelty(width={novelty}, consider_only_novel_states=true, reset_after_progress={reset}), break_ties_randomly={random}, clear_after_progress={clear})]), cost_type=one, reopen_closed=false)"""])
    for novelty in [1, 2]
