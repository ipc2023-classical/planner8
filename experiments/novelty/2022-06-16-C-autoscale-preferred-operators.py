...
CONFIGS = [
    ("01-ff-prefops", [
        ...
        "--search", """lazy(single(hff), cost_type=one, preferred=[hff], reopen_closed=false)"""]),
] + [
    (f"ff-prefops-novelty-{novelty}{'-random' if random else ''}{'-reset' if reset else ''}-{handle_progress}", [
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--search", f"""lazy(alt([single(hff), novelty_open_list(novelty(width={novelty}, consider_only_novel_states=true, reset_after_progress={reset}), break_ties_randomly={random}, handle_progress={handle_progress})]), cost_type=one, preferred=[hff], reopen_closed=false)"""])
    for novelty in [1, 2]
    ...
    ("01-lmcount-prefops", [
        ...
        "--search", """lazy(single(hlm), cost_type=one, preferred=[hlm], reopen_closed=false)"""]),
] + [
    (f"ff-lmcount-prefops-novelty-{novelty}{'-random' if random else ''}{'-reset' if reset else ''}-{handle_progress}", [
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--evaluator", "hlm=lmcount(lm_factory=lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(one),pref=false)",
        "--search", f"""lazy(alt([single(hff), single(hlm), novelty_open_list(novelty(width={novelty}, consider_only_novel_states=true, reset_after_progress={reset}), break_ties_randomly={random}, handle_progress={handle_progress})]), cost_type=one, preferred=[hff,hlm], reopen_closed=false)"""])
    for novelty in [1, 2]
    for reset in [True, False]
    for handle_progress in ["clear", "move"]
    for random in [False]
