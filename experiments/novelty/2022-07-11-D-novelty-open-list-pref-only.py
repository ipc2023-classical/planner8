...
CONFIGS = [(f"lama-novelty-{novelty}-pref-only-{'-random' if random else ''}{'-reset' if reset else ''}-{handle_progress}-prefops", [
        "--evaluator",
    ...
    (f"lama-type-based-novelty-{novelty}-pref-only-{'-random' if random else ''}{'-reset' if reset else ''}-{handle_progress}-prefops", [
        "--evaluator",
        "hlm=lmcount(lm_factory=lm_reasonable_orders_hps(lm_rhw()),transform=adapt_costs(one),pref=false)",
        "--evaluator", "hff=ff(transform=adapt_costs(one))",
        "--evaluator", f"novelty_eval=novelty(width={novelty}, consider_only_novel_states=true, reset_after_progress={reset})",
