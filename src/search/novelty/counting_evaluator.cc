...
namespace novelty {
CountingEvaluator::CountingEvaluator(const Options &opts)
    : Heuristic(opts),
...
int CountingEvaluator::get_and_increase_fact_pair_novelty(int fact_id1, int fact_id2) {
...
int CountingEvaluator::compute_and_increase_novelty(const State &state) {
...
int CountingEvaluator::compute_heuristic(const State &state) {
...
static shared_ptr<Heuristic> _parse(OptionParser &parser) {
    ...
    if (parser.dry_run())
        ...
        return make_shared<CountingEvaluator>(opts);
}

static Plugin<Evaluator> _plugin("count", _parse);
}
